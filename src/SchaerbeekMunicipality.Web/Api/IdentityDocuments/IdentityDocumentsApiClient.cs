    ...
    : MunicipalApiClientBase(httpClient, currentOfficer), IIdentityDocumentsApi
{
    ...
    public Task<IReadOnlyList<DocumentRequestCaseListItem>> ListRequestsAsync(CancellationToken cancellationToken = default) =>
        GetJsonAsync<IReadOnlyList<DocumentRequestCaseListItem>>($"{BasePath}/requests", cancellationToken);

    public Task<OpenDocumentRequestCaseResponse> OpenRequestAsync(
        OpenDocumentRequestCaseRequest request,
        CancellationToken cancellationToken = default) =>
        PostJsonAsync<OpenDocumentRequestCaseRequest, OpenDocumentRequestCaseResponse>(
            $"{BasePath}/requests",
            request,
            cancellationToken);
    ...
    public Task<DocumentRequestCaseDetailDto> GetRequestAsync(Guid id, CancellationToken cancellationToken = default) =>
        GetJsonAsync<DocumentRequestCaseDetailDto>($"{BasePath}/requests/{id}", cancellationToken);

    public Task<ClaimDocumentRequestCaseResponse> ClaimRequestAsync(Guid id, CancellationToken cancellationToken = default) =>
        PostJsonAsync<ClaimDocumentRequestCaseResponse>($"{BasePath}/requests/{id}/claim", cancellationToken);

    public Task<ClaimDocumentRequestCaseResponse?> TryAutoClaimRequestAsync(Guid id, CancellationToken cancellationToken = default) =>
        PostJsonOptionalAsync<ClaimDocumentRequestCaseResponse>($"{BasePath}/requests/{id}/auto-claim", cancellationToken);

    public Task<ReleaseCaseLockResponse> ReleaseCaseLockAsync(Guid id, CancellationToken cancellationToken = default) =>
        PostJsonAsync<ReleaseCaseLockResponse>($"{BasePath}/requests/{id}/release-lock", cancellationToken);
    ...
    public Task<AttachApplicantPhotoResponse> AttachApplicantPhotoAsync(
        ...
        string fileName,
        CancellationToken cancellationToken = default) =>
        PostMultipartFileAsync<AttachApplicantPhotoResponse>(
    ...
    public Task<Stream> DownloadDocumentAsync(
        ...
        Guid documentId,
        CancellationToken cancellationToken = default) =>
        DownloadStreamAsync($"{BasePath}/requests/{id}/documents/{documentId}", cancellationToken);

    public Task<RemoveDocumentResponse> RemoveDocumentAsync(
        Guid id,
        Guid documentId,
        CancellationToken cancellationToken = default) =>
        DeleteJsonAsync<RemoveDocumentResponse>($"{BasePath}/requests/{id}/documents/{documentId}", cancellationToken);

    public Task<RecordFeePaymentResponse> RecordFeePaymentAsync(
        Guid id,
        RecordFeePaymentRequest request,
        CancellationToken cancellationToken = default) =>
        PostJsonAsync<RecordFeePaymentRequest, RecordFeePaymentResponse>(
            $"{BasePath}/requests/{id}/fee-payment",
            request,
            cancellationToken);

    public Task<AdvanceDocumentRequestStatusResponse> AdvanceStatusAsync(Guid id, CancellationToken cancellationToken = default) =>
        PostJsonAsync<AdvanceDocumentRequestStatusResponse>($"{BasePath}/requests/{id}/advance", cancellationToken);

    public Task<IssueDocumentResponse> IssueDocumentAsync(Guid id, CancellationToken cancellationToken = default) =>
        PostJsonAsync<IssueDocumentResponse>($"{BasePath}/requests/{id}/issue", cancellationToken);

    public Task<CancelDocumentRequestResponse> CancelRequestAsync(
        Guid id,
        CancelDocumentRequestRequest request,
        CancellationToken cancellationToken = default) =>
        PostJsonAsync<CancelDocumentRequestRequest, CancelDocumentRequestResponse>(
