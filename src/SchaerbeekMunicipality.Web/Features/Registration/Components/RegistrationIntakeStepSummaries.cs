...
public static class RegistrationIntakeStepSummaries
{
    ...
    public static IReadOnlySet<RegistrationIntakeStep> GetDefaultExpandedSteps(RegistrationCaseDetailDto dto)
    {
        ...
        if (!IsComplete(RegistrationIntakeStep.Identity, dto)) return expanded;

        foreach (var step in StepOrder)
        {
            if (step == RegistrationIntakeStep.Identity) continue;

            if (!IsVisible(step, dto)) continue;
    ...
    public static string GetSummary(RegistrationIntakeStep step, RegistrationCaseDetailDto dto)
    {
        return step switch
        {
    ...
    {
        if (dto.Person is null) return "Not recorded";

        return
            $"{dto.Person.GivenName} {dto.Person.FamilyName} · born {dto.Person.BirthDate:d} · {dto.Person.Nationality}";
    }

    private static string FormatLegalResidenceSummary(RegistrationCaseDetailDto dto)
    {
        if (dto.ResidenceCategory is null) return "Not classified";

        var summary = FormatResidenceCategory(dto.ResidenceCategory.Value);

        if (dto.ResidencePermit is { } permit) summary += $" · permit until {permit.ValidUntil:d}";
    ...
    {
        if (dto.DeclaredAddress is null) return "Not declared";

        var address = dto.DeclaredAddress;
        var line = $"{address.Street} {address.HouseNumber}";
        if (!string.IsNullOrWhiteSpace(address.Box)) line += $"/{address.Box}";
    ...
    {
        if (dto.CivilStatus is null) return "Not recorded";
    ...
    {
        if (dto.Person?.BirthInformation is null) return "Not recorded";

        var birth = dto.Person.BirthInformation;
        var summary = birth.BirthPlace;

        if (!string.IsNullOrWhiteSpace(birth.BirthCountry)) summary += $", {birth.BirthCountry}";

        if (!dto.Checklist.BirthEvidenceEstablished) summary += " · certificate missing";
    ...
    {
        if (dto.ActivePoliceVerification is { } pending) return $"Awaiting result · visit {pending.AttemptNumber}";
    ...
    private static string FormatResidenceCategory(ResidenceCategory category)
    {
    ...
    private static string FormatCivilStatus(CivilStatus status)
    {
    ...
    private static string FormatPoliceResult(PoliceVerificationResult result)
    {
